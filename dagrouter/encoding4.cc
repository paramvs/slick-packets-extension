...
static char rcsid[] = "$Id: encoding4.cc 3 2012-01-03 01:11:28Z cauthu@gmail.com $";
...
static int
consumeDetourPath(const bitstring encoding,
                  const unsigned int& encodingLen,
                  unsigned int& curBitNum)
{
    int retcode = getVarLenValFromBitstring(
        encoding, encodingLen, curBitNum, numSetBits2Len,
        ARRAY_LENGTH(numSetBits2Len), &);
}
                     ...
                     unsigned int* ret_newEncodingLen)
{
    ...
    *ret_action = action_inval;
    ...
    linkId = getValFromBitstring(bs, curBitNum, LEN);
